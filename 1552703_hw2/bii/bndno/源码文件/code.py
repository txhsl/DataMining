...
def input():
    ...
    data_extra = grouped.drop(['rank', 'sldat', 'bndno'], axis=1).drop_duplicates('uid').set_index('uid')
...
def input_new():
    ...
    # add timestamp and vipno
    data_extra = grouped.drop(['rank', 'sldatime', 'bndno'], axis=1).drop_duplicates('uid').set_index('uid')
    data_set = pd.concat([data_set,data_extra], axis=1, join='inner').reset_index(drop=True).sort_values(['vipno','timestamp']).drop(['timestamp'], axis=1)
