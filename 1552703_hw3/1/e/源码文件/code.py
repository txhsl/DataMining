...
if __name__ == '__main__':
    ...
    # find the similar clf
    topkps = []
    top_group = data_set[0]
    ed_max = 0
    ...
    for topkm in topkms:
        ...
        for group in data_set:
            # not the same one
            if group.size != topkm.size:
                # use euclidean_distances
                distances = euclidean_distances(topkm.fillna(0), group.fillna(0))
                if distances.mean() > ed_max:
                    
                    ed_max = distances.mean()
                    top_group = group
        ...
        # merge
        group = topkm
        new_group = pd.concat([topkm, top_group])
